...
def verify(request):
    context = {"page_title": "OTP Verification"}
    return render(request, "voting/voter/verify.html", context)


def resend_otp(request):
    """API For SMS
    https://www.multitexter.com/ API to send SMS
    ...
    if settings.SEND_OTP:
        if voter.otp_sent >= 3:
            ...
            if otp is None:
                ...
                voter.save()
            try:
                msg = "Dear " + str(user) + ", kindly use " + str(otp) + " as your OTP"
...
def bypass_otp():
    Voter.objects.all().filter(otp=None, verified=False).update(
